...
def exibir_menu_imagens(coluna):
  with coluna:
    ...
    Selecione as imagens para gerar um PDF.
...
def gerar_arquivo_pdf_com_imagens(imagens):
  ...
  outras_imagens = imagens_pillow[1:]

  with tempfile.TemporaryDirectory() as tempdir:
    nome_arquivo = Path(tempdir) / 'temp.pdf'
    primeira_imagem.save(nome_arquivo, save_all=True, append_images=outras_imagens)
  ...
  for pagina in pdf_imagens.pages:
    ...
    tx = (pagina_em_branco.mediabox.right  - pagina_em_branco.mediabox.right * scale) / 2
    ty = (pagina_em_branco.mediabox.top  - pagina_em_branco.mediabox.top * scale) / 2
